...
export default function SkillTab() {
  ...
  const meta = skillsQuery.data?.data?.meta;
        ...
        { id: editingItem.id, data: { name } },
        {
          onSuccess: () => {
            setNotice({
              ...
              message: t("systemSetting.skill.notifications.updateSuccess"),
            });
            handleCloseModal();
          },
          onError: () => {
            setNotice({
              ...
              message: t("systemSetting.skill.notifications.updateError"),
            });
          },
        },
      );
        ...
        { name },
        {
          onSuccess: () => {
            setNotice({
              variant: "success",
              title: t("systemSetting.notifications.successTitle"),
              message: t("systemSetting.skill.notifications.createSuccess"),
            });
            handleCloseModal();
          },
          onError: () => {
            setNotice({
              variant: "error",
              title: t("systemSetting.notifications.errorTitle"),
              message: t("systemSetting.skill.notifications.createError"),
            });
          },
        },
      );
    ...
    deleteMutation.mutate(deleteTarget.id, {
      onSuccess: () => {
        setNotice({
          variant: "success",
          title: t("systemSetting.notifications.successTitle"),
          message: t("systemSetting.skill.notifications.deleteSuccess"),
        });
      ...
      onError: () => {
        setNotice({
          variant: "error",
          title: t("systemSetting.notifications.errorTitle"),
          message: t("systemSetting.skill.notifications.deleteError"),
        });
        setDeleteTarget(null);
      },
    });
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="flex flex-col gap-4 rounded bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-1 items-center gap-3 rounded border bg-white px-4">
          ...
          <Input
            ...
            placeholder={t("systemSetting.skill.searchPlaceholder")}
            value={keyword}
        ...
        <Button onClick={handleOpenCreate} className="gap-2">
          ...
          {t("systemSetting.skill.addButton")}
        </Button>
      </section>
            ...
            <thead className="bg-primary text-[12px] font-semibold uppercase text-white">
              <tr>
                ...
                <th className="px-6 py-4">
                  {t("systemSetting.skill.table.name")}
                </th>
                <th className="px-6 py-4">
                  {t("systemSetting.skill.table.createdAt")}
                </th>
                <th className="px-6 py-4">
                  {t("systemSetting.skill.table.updatedAt")}
                </th>
                <th className="px-6 py-4 text-right">
                  {t("systemSetting.skill.table.actions")}
                </th>
              </tr>
            </thead>
      ...
      <NotificationPopup
        open={!!deleteTarget}
        variant="confirm"
        title={t("systemSetting.skill.confirm.deleteTitle")}
        message={t("systemSetting.skill.confirm.deleteMessage", {
