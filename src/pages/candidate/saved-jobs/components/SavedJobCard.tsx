...
const SavedJobCard = ({ job, isUnsaving, onUnsave }: SavedJobCardProps) => {
  ...
  const applyLabel = t("savedJobs.card.applyNow");
      ...
      <div className="flex w-full flex-1 items-center gap-4">
        {job.logoUrl ? (
          <img
            ...
            alt={
              job.logoAlt ??
              t("savedJobs.card.logoAlt", { company: job.company })
            }
      ...
      <div className="flex w-full flex-col gap-4 sm:w-auto sm:items-end">
        {job.isClosed ? (
          <span className="flex items-center gap-2 text-sm font-semibold text-destructive">
            ...
            {t("savedJobs.card.jobClosed")}
          </span>
        ) : (
          <span className="text-sm font-medium text-primary">
            {typeof job.daysLeft === "number"
              ? t("savedJobs.card.daysLeft", { count: job.daysLeft })
              : t("savedJobs.card.openForApplications")}
          </span>
        )}
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
          ...
          <div className="flex w-full items-center gap-3 sm:w-auto">
            ...
            >
              {job.isApplied ? (
                t("savedJobs.card.applied")
              ) : job.isClosed ? (
                applyLabel
              ) : (
                <Link to={`/jobs/detail/${job.id}`}>{applyLabel}</Link>
              )}
            </Button>
            <Button
              variant="ghost"
              ...
              aria-label={t("savedJobs.card.unsave")}
              disabled={isUnsaving}
